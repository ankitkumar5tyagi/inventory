<x-Layout>
    ...
    <form action="{{ route('supplier.update', $supplier) }}" method="POST">
        ...
        <div class="inputdiv">
            ...
            <input type="text" name="pan" value="{{ $supplier->pan }}" id="pan" @error('pan') style="border-color: red;"@enderror>
            @error('pan')
                <span class="text-red-600">{{$message}}</span>
            @enderror
        </div>
        <div class="inputdiv">
            ...
            <input type="text" name="gst" value="{{ $supplier->gst }}" id="gst" @error('gst') style="border-color: red;"@enderror>
            @error('gst')
                <span class="text-red-600">{{$message}}</span>
            @enderror
        </div>
