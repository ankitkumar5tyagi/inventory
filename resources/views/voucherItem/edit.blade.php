<x-Layout>
    ...
    <form action="{{ route('voucherItem.update', $voucherItem) }}" method="POST">
        @csrf
        @method('PUT')

        <div class="inputdiv">
            <label for="voucher_entry_id" class="block text-sm font-medium text-gray-700">Voucher No: </label>
            <select type="text" value="{{ $voucherItem->voucher_entry_id }}" name="voucher_entry_id" id="voucher_entry_id" @error('voucher_entry_id') style="border-color: red;"@enderror onchange="updateForm()">
        ...
        <div class="inputdiv">
            ...
            <select type="text" name="item_id" value="{{ $voucherItem->item_id }}" id="item_id" @error('item_id') style="border-color: red;"@enderror onchange="updateUOM(this)">
